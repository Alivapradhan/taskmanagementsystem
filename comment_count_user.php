<?php
// Check if 'task_id' is set in the URL query string
if (isset($_GET['task_id']) && !empty($_GET['task_id'])) {
    // 'task_id' is present in the URL
    $task_id = $_GET['task_id'];
} else {
    // If 'task_id' is not set or is empty, show a friendly error message or set a default value   
    echo "<p>Error: task_id is missing or invalid in the URL.</p>";
    exit;  // Exit the script to prevent further execution
}

require '../Classes/init.php'; // Ensure that init.php is correctly loading the necessary classes
$func = new Operation();
$display = 1;

// Fetch the comments from the database based on task_id and display status
$result = $func->select_with_multiple_condition_orderby(
    array('*'),
    'comments',
